      ...
      <el-container>
        ...
        <el-aside width="250px">
          <div class="components-list">
            <template v-if="postersFields.length">
              <div class="widget-cate">poster</div>
              <!--列出基础字段-->
              <draggable tag="ul" :list="postersComponents"
              ...
              >
                ...
                <li v-if="postersFields.indexOf(item.type)>=0" class="form-edit-widget-label" :class="{'no-put': item.type == 'divider'}" v-for="(item, index) in postersComponents" :key="index">
                  <a>
        ...
        <el-container class="center-container" direction="vertical">
          ...
          <el-main :class="{'widget-empty': widgetForm.list.length == 0}">
            ...
            <!-- <widget-post v-if="!resetJson"  ref="widgetForm" :data="widgetForm" :select.sync="widgetFormSelect"></widget-post> -->
          </el-main>
        </el-container>
...
import {postersComponents} from './componentsConfig.js'
...
export default {
  ...
  props: {
    ...
    postersFields: {
      type: Array,
      default: () => ['input', 'textarea', 'number', 'radio', 'checkbox', 'time', 'date', 'rate', 'color', 'select', 'switch', 'slider', 'text','blank', 'imgupload', 'editor', 'cascader']
    },
    basicFields: {
      type: Array,
  ...
  mounted () {
      //console.log(this.$refs.iframe2.contentWindow,1111);
      //this.iframeWin=this.$refs.iframe2.contentWindow;
    //  console.log(this.$refs.iframe,33);
    //this.iframeWin.postMessage(JSON.stringify(this.widgetForm),'http://192.168.5.49:8080/nuxt/activity/20190922/formMakeTest');
    Bus.$on('userImg',data=>{
          //向子页面传递命令
          console.log(this.$refs.iframe,123);
            this.iframeWin2=this.$refs.iframe.contentWindow ;
            console.log(this.iframeWin,222);
           this.iframeWin2.postMessage(JSON.stringify(this.widgetForm),'http://192.168.5.49:8080/nuxt/activity/20190922/formMakeTest');
    })
...
//          //代码能执行到这里说明已经载入成功完毕了
//              this.iframeWin=this.$refs.iframe.contentDocument ;
//              console.log(this.iframeWin);
//              this.iframeWin.postMessage(JSON.stringify(this.widgetForm),'http://192.168.5.49:8080');
        ...
        list: [
          {
            ...
            "options": {
              ...
              "hidden": false,
              "dataBind": true,
              "remoteFunc": "func_1574843468267"
            },
            "name": "文字",
            "key": "123",
            "model": "text_1574843468267",
            "rules": []
          }
        ],
        config: {
